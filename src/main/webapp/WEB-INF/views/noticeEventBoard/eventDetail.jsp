	...
	<div style="margin:0 auto; width:50%">
		<table class="table">
		  ...
		  <tbody>
		    ...
		    <tr>
		    	<td colspan="3">${EventB.eventBoardContents}</td>
		    </tr>
		  </tbody>
		</table>
      <c:if test="${member != null}">
	      <c:if test="${member.adminCk == 1}">
			<div>
				<button onclick="eventListBtn()">글목록(notice)</button>
				<button onclick="eventUpdateBtn()">글수정(notice)</button>
				<button onclick="eventDeleteBtn()">글삭제(notice)</button>
			</div>
			</c:if>
		</c:if>
		<hr>
